    ...
    while(true){
        ...
        if(strcmp(input, "end") == 0) break;
            ...
            while(!isEmpty(readyQueue)){
                QNode* temp = deQueue(readyQueue);

                if(temp->priority == 1){
                    enQueue(q1, temp->pid, temp->command, temp->priority, 0);
                    ++numProcess1;
                }

                else if(temp->priority == 2){
                    enQueue(q2, temp->pid, temp->command, temp->priority, 0);
                    ++numProcess2;
                }

                else if(temp->priority == 3){
                    enQueue(q3, temp->pid, temp->command, temp->priority, 0);
                    ++numProcess3;
                }

                else{
                    enQueue(q4, temp->pid, temp->command, temp->priority, 0);
                    ++numProcess4;
                }
            }

            while(!isEmpty(q1) || !isEmpty(q2) || !isEmpty(q3) || !isEmpty(q4)){
            ...
            if(create_process == 0){
                ...
                enQueue(readyQueue, create_process, exact_command, priority, 0);
            }
