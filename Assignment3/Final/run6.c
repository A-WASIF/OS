...
void printHistory(HistoryEntry* history, int historyIndex) {
    printf("\nCommand History(Command, Execution Time, Waiting Time):\n");
    printf("*****************************************\n");
    for (int i = 0; i < historyIndex; i++) {
        printf("%d: %s      %d sec.     %dsec. \n", i + 1, history[i].command, history[i].execution_time, history[i].wait_time);
...
QNode* newNode(int k, char* command, int priority, int exe_time)
{
...
void enQueue(struct Queue* q, int k, char* command, int priority, int exe_time)
{
...
void scheduler(int numProcess, int NCPU, int TSLICE, struct Queue* q, struct Queue* Q, int *Qnum, HistoryEntry history[], int historyIndex);

void initializeWaitTime(HistoryEntry history[], int historyIndex, int priority, int TSLICE, int prev_numProcess){
    for(int i = 0; i < historyIndex; i++){
        if(history[i].priority == priority) history[i].wait_time = prev_numProcess * TSLICE;
    }
}

void calWaitTime(struct Queue* q, int TSLICE, int NCPU, HistoryEntry history[], int historyIndex){
    int ind = 0;
    QNode* current = q->front;
    while (current != NULL) {
        for(int i = 0; i < historyIndex; i++){
...
void prepAndLaunch_schduler(struct Queue* q1, struct Queue* q2, struct Queue* q3, struct Queue* q4, int TSLICE, int NCPU, HistoryEntry history[], int historyIndex){
    ...
    initializeWaitTime(history, historyIndex, 2, TSLICE, numProcess1);
    initializeWaitTime(history, historyIndex, 3, TSLICE, (numProcess2 + numProcess1));
    initializeWaitTime(history, historyIndex, 4, TSLICE, numProcess3 + numProcess2 + numProcess1);
...
void scheduler(int numProcess, int NCPU, int TSLICE, struct Queue* q, struct Queue* Q, int *Qnum, HistoryEntry history[], int historyIndex){
    // wait_time(q, TSLICE, history, historyIndex);
    while(!isEmpty(q)){
