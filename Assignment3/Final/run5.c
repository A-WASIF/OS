...
void printHistory(HistoryEntry* history, int historyIndex) {
    printf("\nCommand History(Command, Execution Time):\n");
    printf("*****************************************\n");
...
QNode* newNode(int k, char* command, int priority, int time)
{
...
void enQueue(struct Queue* q, int k, char* command, int priority, int time)
{
	// Create a new LL node
	QNode* temp = newNode(k, command, priority, time);
        ...
        for(int i = 0; i < process; i++){
            ...
            if (child_pid == 0) {
                ...
                if(state == 0){
                    ...
                    --numProcess;
                    for(int i = 0; i < historyIndex; i++){
                        if (strcmp(history[i].command, temp->command) == 0) {
                            history[i].execution_time = temp->execution_time;
                            break;
                        }
                    }
                    printf("Execution Time of %u : %d\n", temp->pid, temp->execution_time);
                }
            }
